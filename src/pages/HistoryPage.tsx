  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          ...
          {statusFilters.map((f) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${statusFilter === f.value ? 'bg-brand-600 text-white' : 'bg-surface-100 text-surface-600 hover:bg-surface-200 dark:bg-surface-700 dark:text-surface-300 dark:hover:bg-surface-600'}`}
            >
              {f.label}
            </button>
          ))}
      ...
      <div className="card overflow-hidden">
        <div className="hidden md:grid grid-cols-12 gap-4 px-6 py-3 bg-surface-50 dark:bg-surface-800 border-b border-surface-200 dark:border-surface-700 text-xs font-medium text-surface-500 dark:text-surface-400 uppercase tracking-wider">
        ...
        <div className="divide-y divide-surface-100 dark:divide-surface-700">
          {filtered.length === 0 && (
            <div className="px-6 py-12 text-center text-surface-400 dark:text-surface-500">該当する送信履歴がありません</div>
          )}
          {filtered.map((d) => {
            ...
            return (
              <Link key={d.id} to={`/history/${d.id}`} className="grid grid-cols-1 md:grid-cols-12 gap-2 md:gap-4 px-6 py-4 hover:bg-surface-50 dark:hover:bg-surface-700/50 transition-colors items-center group">
                <div className="col-span-4 flex items-center gap-3 min-w-0">
                  <div className={`rounded-lg p-2 shrink-0 ${d.password_protected ? 'bg-amber-50 dark:bg-amber-900/20' : 'bg-brand-50 dark:bg-brand-900/20'}`}>
                    {d.password_protected ? <Lock className="h-4 w-4 text-amber-600 dark:text-amber-400" /> : <FileText className="h-4 w-4 text-brand-600 dark:text-brand-400" />}
                  </div>
                ...
                <div className="col-span-2 text-sm text-surface-600 dark:text-surface-300">
                  ...
                  {recipientCount > 1 && <span className="text-surface-400 dark:text-surface-500 ml-1">+{recipientCount - 1}</span>}
                </div>
                <div className="col-span-2 text-sm text-surface-600 dark:text-surface-300 flex items-center gap-1.5">
                  <Download className="h-3.5 w-3.5 text-surface-400 dark:text-surface-500" />
                  {fileCount}件 ({formatFileSize(totalSize)})
                  <span className="text-surface-400 dark:text-surface-500 text-xs ml-1">{dlCount}/{recipientCount} DL</span>
                </div>
                <div className="col-span-2 text-sm text-surface-500 dark:text-surface-400">
                  {formatDate(d.created_at)}
                  {!isExpired(d.expires_at) && expDays <= 3 && (
                    <span className="block text-xs text-amber-600 dark:text-amber-400 mt-0.5">残り{expDays}日</span>
                  )}
                </div>
